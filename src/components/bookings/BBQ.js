...
import BBQDetails from "./BBQDetails";

const BBQ = ({addBooking}) => {
    ...
    const handleCloseDialog = () => {
        setShowDialog(false);
        addBooking("BBQ Grill", "bbq.png", "#F97979", fullDate, selectedTime)
    }
    ...
    for (let i=0; i < 1;i++) {
        details.push(<BBQDetails index={i+1} dates={dates} setDate={setDate} handleClick={handleClick} times={times} />)
    }

    return (
        <Box>
            <Accordion sx={{backgroundColor: "#F97979"}}>
            ...
            <Dialog open={showDialog} onClose={handleCloseDialog}>
                <DialogContent sx={{borderStyle:"solid", borderWidth:"5px", borderRadius:"5px"}}>
                    ...
                    <Typography sx={{fontFamily:"Source Sans 3", fontWeight:"700", fontSize:"20px", color:"#3D5CAC", textAlign:"center"}}>BBQ Grill</Typography>
