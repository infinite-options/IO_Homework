...
const Washer = ({addBooking}) => {
    const [show, setShow] = useState(false);
    const [selectedTime, setSelectedTime] = useState("");
    const [washerNum, setWasherNum] = useState(0)
    ...
    const handleCloseDialog = () => {
        setShowDialog(false);
        addBooking("Washers", "washer.png", "#79CBF9", fullDate, selectedTime, washerNum)
    }
    ...
    for (let i=0; i < num_washers;i++) {
        details.push(<WasherDetails index={i+1} dates={dates} setDate={setDate} handleClick={handleClick} washerNum={washerNum} times={times} />)
    }

    return (
        <Box>
            <Accordion sx={{backgroundColor: "#79CBF9"}}>
            ...
            <Dialog open={showDialog} onClose={handleCloseDialog}>
                <DialogContent sx={{borderStyle:"solid", borderWidth:"5px", borderRadius:"5px"}}>
                    ...
                    <Typography sx={{fontFamily:"Source Sans 3", fontWeight:"700", fontSize:"20px", color:"#3D5CAC", textAlign:"center"}}>Washing Machine #{washerNum}</Typography>
