...
const WasherBookings = ({numBookings, washerNum, utilization, persons, setTitle, setPersons}) => {
    const handleClick=()=> {
        setTitle("Washing Machine #" + washerNum)
        setPersons(persons)
    }
    if (!persons) {
        persons=[];
                        ...
                        <Accordion onClick={handleClick} sx={{marginLeft:"15px", marginRight:"15px", width:"95%"}} >
                            <AccordionSummary expandIcon={<img src="arrow.png" alt="expand" />}>
                                <Grid item sx={{position:"relative", display:"inline-block"}}>
                                    ...
                                    <Typography sx={{position:"absolute", top:"30%", left:"35%", fontFamily:"Source Sans 3", fontWeight:"700", fontSize:"25px"}}>{washerNum}</Typography>
                                </Grid>
