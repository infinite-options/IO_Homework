...
const BbqBookings = ({ numBookings, utilization, persons, setTitle, setPersons }) => {
    const handleClick = () => {
        setTitle("BBQ Grill");
        setPersons(persons);
    };
    ...
    return (
        <Grid item xs={12} container sx={{ mb: "10px", justifyContent: "center" }}>
            <Accordion onClick={handleClick} sx={{ marginLeft: "15px", marginRight: "15px", transition: 'none' }}>
                <AccordionSummary expandIcon={<img src="arrow.png" alt="expand" />} >
                    <Grid item sx={{ position: "relative", display: "inline-block" }}>
                        <img src="bbq.png" alt="bbq" style={{ height: "50px" }} />
                    </Grid>
